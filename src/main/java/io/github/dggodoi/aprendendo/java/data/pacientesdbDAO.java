 ...
 * @ kmx
 */
public class pacientesdbDAO {
    private final ConexaoJDBC con;
    SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd");
    
     public pacientesdbDAO() throws SQLException, ClassNotFoundException{
 ...
 //anunciante,valor,destaque,imagem_url
    public Long insert(Paciente oferta) throws SQLException, ClassNotFoundException, ParseException{
        Long id = null;
        String sqlQuery = "INSERT INTO pacientesdb(pacientenome,pacientesexo,pacientedatanasc,pacientetipoexame,medicosolicitante,especmedica,hda,telmedico,pacienteconvenio,pacientedataexame,pacienteid)"
    ...
    public int delete(long id) throws SQLException, ClassNotFoundException{
        ...
        String sqlQuery = "DELETE FROM pacientesdb WHERE pacienteid = ?";
        try{
            PreparedStatement stmt = this.con.getConnection().prepareStatement(sqlQuery);
            stmt.setLong(1,id);
    ...
    public Paciente select(long id) throws SQLException, ClassNotFoundException{
        String sqlQuery = "SELECT * FROM pacientesdb WHERE pacienteid = ?";
        try{
            PreparedStatement stmt = this.con.getConnection().prepareStatement(sqlQuery);
            stmt.setLong(1,id);
            ResultSet rs = stmt.executeQuery();
    ...
    public Paciente selectData(String data) throws SQLException, ClassNotFoundException{
        String sqlQuery = "SELECT * FROM pacientesdb WHERE CAST(pacientedataexame AS DATE) = ?";
        try{
            PreparedStatement stmt = this.con.getConnection().prepareStatement(sqlQuery);
    ...
    private Paciente parser(ResultSet resultSet) throws SQLException{
                ...
                String text = df.format(resultSet.getDate("pacientedataexame"));
    ...
    public List<Paciente> listAll(String data) throws SQLException, ClassNotFoundException, ParseException{
        String sqlQuery = "SELECT * FROM pacientesdb WHERE CAST(pacientedataexame AS DATE) = ?";
         Date dataE = formatter.parse(data);
        java.sql.Date sqlDate = new java.sql.Date(dataE.getTime());
